export async function fetchPostData(slug) {
  try {
    const response = await fetch(
      `${process.env.NEXT_PRIVATE_URL}/api/posts/${slug}?populate[0]=author.avatar&populate[1]=Categories&populate[2]=novelas`,
      {
        headers: {
    ...
    const post = {
      ...
      Categories: result.data.attributes.Categories?.data
        ? result.data.attributes.Categories.data.map((Category) => ({
            id: Category.id,
