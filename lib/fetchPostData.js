export async function fetchPostData(slug) {
  try {
    const response = await fetch(
      `${process.env.NEXT_PRIVATE_URL}/api/posts/${slug}?populate[0]=author.avatar&populate[1]=categorias&populate[2]=novelas`,
      {
        headers: {
    ...
    const post = {
      ...
      author: result.data.attributes.author?.data
        ...
        : null,
      categorias: result.data.attributes.categorias?.data
        ? result.data.attributes.categorias.data.map((categoria) => ({
            id: categoria.id,
            type: categoria.attributes.type,
          }))
        : [],
